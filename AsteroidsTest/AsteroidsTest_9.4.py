...
WINDOW_CAPTION = 'Asteroids! Test version 9.4'
...
##        v--- This part is to make the screen perfectly centered in the middle of the map, regardless of map width. It does this by adjusting map minimum and maximum parameters.
DIFF_BETWEEN_MAP_AND_SCREEN_WIDTH, DIFF_BETWEEN_MAP_AND_SCREEN_HEIGHT = (MAP_X2 - SCREEN_WIDTH), (MAP_Y2 - SCREEN_HEIGHT)

MAP_X, MAP_X2 = (MAP_X - (DIFF_BETWEEN_MAP_AND_SCREEN_WIDTH / 2)), (MAP_X2 - (DIFF_BETWEEN_MAP_AND_SCREEN_WIDTH / 2))
MAP_Y, MAP_Y2 = (MAP_Y - (DIFF_BETWEEN_MAP_AND_SCREEN_HEIGHT / 2)), (MAP_Y2 - (DIFF_BETWEEN_MAP_AND_SCREEN_HEIGHT / 2))


PLAYING_FIELD_X, PLAYING_FIELD_Y = 0, 0    # Screen dimensions determine the most you can see on the screen. If you want the playing field to be smaller (or bigger...) than the screen, that should be altered.
PLAYING_FIELD_X2, PLAYING_FIELD_Y2 = SCREEN_WIDTH, SCREEN_HEIGHT    

## Game mechanics constants ---v

## Does the player ship maintain its angular velocity when not acted upon by an outside force?
PLAYER_HAS_ANGULAR_VELOCITY = False

## Is the player's ship suspended in a motion-resisting aether, or are they in outer space? Slows the ship down when not accelerating.
...
class GameObject:
    ...
    def __init__(self, starting_x, starting_y, x_velocity, y_velocity, angular_velocity, current_angle_in_degrees=0, size=1, color=WHITE, programmatic_object_shape=1, is_asteroid=False, is_owned_by_player=False, is_shot_object=False, is_alien_ship=False, is_debris_object=False, duration_remaining=None, specific_max_velocity=None):
        ...
        ## GameObjects are not UserInterfaceObjects, though UserInterfaceObjects are GameObjects.
    ...
    def draw(self):
        ...
        draw_programmatic_object(self.x, self.y, self.current_angle_in_degrees, self.programmatic_object_shape, self.color, self.size)
        
        if (((((player_is_accelerating == True) and (player_fired_shot == False)) and (self.is_owned_by_player == True)) and (self.is_shot_object == False) ) and (self.is_user_interface_object == False) ):
            ## If the ship is accelerating, draw exhaust behind the ship.        
            draw_programmatic_object(self.x, self.y, self.current_angle_in_degrees, this_programmatic_object_shape=-2, color=self.color, size=self.size)
    ...
    def move(self):
        ...
        if ((self.is_owned_by_player == False) or (self.is_shot_object == True) or (self.is_alien_ship == True)):
        ...
        elif ((self.is_owned_by_player == True) and (self.is_shot_object == False)):
            ## If it's owned by the player and is NOT a shot object, it must be the player ship, and should bounce off the edges of the playing field (probably == the screen, too).
        ...
        ## [Asteroid] collision with [shot, player ship, alien ship]-detection. Note this section uses "shot" to refer to all of these things.
        ##            Is this a shot?    IF NOT...   is it a player-owned object AND not a shot (ie, the player's ship) ... AND also can the player's ship be harmed? IF NOT, is it an alien ship?
        if ( ( (self.is_shot_object == True) or ( ((self.is_owned_by_player == True) and (self.is_shot_object == False)) and (PLAYER_SHIP_IS_INVULNERABLE == False) ) ) or (self.is_alien_ship == True) ):
            for each_asteroid in asteroid_objects_array:
            
                x_distance_between_shot_and_rock = self.x - each_asteroid.x
                y_distance_between_shot_and_rock = self.y - each_asteroid.y
                
                distance_between_them_as_hypotenuse = math.sqrt((x_distance_between_shot_and_rock * x_distance_between_shot_and_rock) + (y_distance_between_shot_and_rock * y_distance_between_shot_and_rock))
                
                #print("DEBUG: distance_between_them_as_hypotenuse == " + str(distance_between_them_as_hypotenuse))
                
                combined_shot_and_asteroid_radius = self.radius + each_asteroid.radius
                
                #print("DEBUG: combined_shot_and_asteroid_radius == " + str(combined_shot_and_asteroid_radius))
                
                if combined_shot_and_asteroid_radius >= distance_between_them_as_hypotenuse:
                
                    #print("DEBUG: Asteroid and shot removal attempt noted.")
                    if self in shot_objects_array:
                        shot_objects_array.remove(self)
                    if self in player_ship_objects_array:
                        ...
                        remove_player_life()
                        ## aaaand spawn a debris cloud! And implement timers for debris cloud+player respawning, asteroid debris clouds, shots etc.
                        ## DONE.
                    
                    if self in alien_ship_objects_array:
                        ## Alien ships colliding with asteroids DO NOT result in ++score. That is a section further down. Follow the "self" usage in all these sections to figure it out.
                        #if self.size == 40:
                        #    score += 200
                        #elif self.size == 20:
                        #    score += 1000
                    ...
                    if each_asteroid in asteroid_objects_array:
                        ...
                        if self.is_owned_by_player == True:
                            ## If the thing destroying the asteroid is_owned_by_player, then it gives score. Otherwise no.
                        ...
                        if ((each_asteroid.size == 50) or (each_asteroid.size == 100)):
                            each_asteroid.break_large_asteroid_into_two_smaller_ones(self.x_velocity, self.y_velocity, self.size)
                        each_asteroid.spawn_debris_cloud()
                        asteroid_objects_array.remove(each_asteroid)
        
        ## [Alien ship] collision with [player ship shot, player ship]-detection. Note that the previous section handled asteroids hitting alien ships and alien ship shots hitting asteroids.
        if self.is_alien_ship == True:
        
            ## Collision with player shots:
            for each_shot_object in shot_objects_array:
                if each_shot_object.is_owned_by_player == True:
                    x_distance_between_alien_and_shot = self.x - each_shot_object.x
                    y_distance_between_alien_and_shot = self.y - each_shot_object.y
                
                    distance_between_them_as_hypotenuse = math.sqrt((x_distance_between_alien_and_shot * x_distance_between_alien_and_shot) + (y_distance_between_alien_and_shot * y_distance_between_alien_and_shot))
                
                    combined_alien_and_shot_radius = self.radius + each_shot_object.radius
                
                    if combined_alien_and_shot_radius >= distance_between_them_as_hypotenuse:
            ...
            for each_player_ship_object in player_ship_objects_array:
                x_distance_between_alien_and_player_ship = self.x - each_player_ship_object.x
                y_distance_between_alien_and_player_ship = self.y - each_player_ship_object.y
                
                distance_between_them_as_hypotenuse = math.sqrt((x_distance_between_alien_and_player_ship * x_distance_between_alien_and_player_ship) + (y_distance_between_alien_and_player_ship * y_distance_between_alien_and_player_ship))
                
                combined_alien_and_player_ship_radius = self.radius + each_player_ship_object.radius
                
                if combined_alien_and_player_ship_radius >= distance_between_them_as_hypotenuse:
                    ...
                    if each_player_ship_object in player_ship_objects_array:
                        each_player_ship_object.spawn_player_ship_debris_cloud()
                        player_ship_objects_array.remove(each_player_ship_object)
                        remove_player_life()
        
        ## [Alien shot] collision with [player ship]-detection.
        elif ((self.is_shot_object == True) and (self.is_owned_by_player == False)):
            for each_player_ship_object in player_ship_objects_array:
                x_distance_between_alien_shot_and_player_ship = self.x - each_player_ship_object.x
                y_distance_between_alien_shot_and_player_ship = self.y - each_player_ship_object.y
                
                distance_between_them_as_hypotenuse = math.sqrt((x_distance_between_alien_shot_and_player_ship * x_distance_between_alien_shot_and_player_ship) + (y_distance_between_alien_shot_and_player_ship * y_distance_between_alien_shot_and_player_ship))
                
                combined_alien_shot_and_player_ship_radius = self.radius + each_player_ship_object.radius
                
                if combined_alien_shot_and_player_ship_radius >= distance_between_them_as_hypotenuse:
        ...
        ##   Should -v be before or after -^  ?  ... I think after. Change direction if near edge of correct area, THEN move? I wonder...
        ...
        ## Asteroids SHOULDN'T bounce off each other. They end up doing this horrid tango of pushing past each other that looks awful anyways.    
            
        if self.is_asteroid == True:
            for each_asteroid in asteroid_objects_array:
                x_distance_between_rock_and_rock = self.x - each_asteroid.x
                y_distance_between_rock_and_rock = self.y - each_asteroid.y
        
                distance_between_them_as_hypotenuse = math.sqrt((x_distance_between_rock_and_rock * x_distance_between_rock_and_rock) + (y_distance_between_rock_and_rock * y_distance_between_rock_and_rock))
                
                combined_rock_and_rock_radius = self.radius + each_asteroid.radius
                
                if combined_rock_and_rock_radius >= distance_between_them_as_hypotenuse:
                    self.x_velocity *= -1
                    self.y_velocity *= -1
                    each_asteroid.x_velocity *= -1
                    each_asteroid.y_velocity *= -1
    ...
    def adjust_all_velocities(self, x_acceleration, y_acceleration, angular_acceleration):
        ...
        ## This returns the actual values that will be incrementing the velocity ---v
    ...
    def hard_velocity_adjustment(self):
        ...
        if (len(player_ship_objects_array) > 0):
            for each_player_ship_object in player_ship_objects_array:
                ...
                x_distance_between_alien_and_player = each_player_ship_object.x - self.x
                y_distance_between_alien_and_player = each_player_ship_object.y - self.y
        ...
        for each_player_ship_object in player_ship_objects_array:    
            ...
            if PLAYER_SHIP_HAS_DRAG == True:
                if ((player_is_accelerating == False) and (player_fired_shot == False)):
                    each_player_ship_object.brake_all_velocities(is_gradual_braking=True)
